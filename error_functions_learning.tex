...
\begin{figure}[h]
	...
	\begin{subfigure}[t]{0.49\linewidth}
		\centering
    \includegraphics[width=0.8\linewidth]{./figs/csp_landscape_complex_zero_big}
		\caption{\csp landscape}\label{fig:csp_landscape} 
	\end{subfigure}
  \hfill
	\begin{subfigure}[t]{0.49\linewidth}
		\centering
    \includegraphics[width=0.8\linewidth]{./figs/efsp_landscape_complex_zero_big}
...
good  error  functions to  describe  constraints.
...
\begin{figure}
  \centering
  \includegraphics[width=0.8\linewidth]{./figs/model_nn}
    ...
    \begin{minipage}{\linewidth}
      ...
      \begin{tabular}{@{}l|c|r|r|r|r@{}}
        ...
        \multirow{4}{*}{Sudoku \(9\times9\)} &
                                               none (\csp)& \cellcolor{Gray}4.2 & \cellcolor{Gray}0.1 & \cellcolor{Gray}13 &\cellcolor{Gray}95\\
                &fast-forward    & \(5e^{-2}\)  & \(5e^{-2}\)  & \(1e^{-2}\) &100\\
                &hard-coded      & \(2e^{-2}\)  & \(2e^{-2}\)  & \(1e^{-2}\) &100\\
                &hand-crafted    & \(2e^{-2}\)  & \(2e^{-2}\)  & \(6e^{-3}\) &100\\
        \midrule
        \multirow{4}{*}{Sudoku \(16\times16\)} &
                                                 none (\csp)& \cellcolor{Gray}- & \cellcolor{Gray}- & \cellcolor{Gray}- &\cellcolor{Gray}0\\
                &fast-forward    & 1.6  & 1.6  & 0.3 &100\\
                &hard-coded      & 1.1 & 1.1 & 0.2 &100\\
                &hand-crafted & 0.6 & 0.6 & 0.1 &100\\
        \midrule
        \multirow{4}{*}{Sudoku \(25\times25\)} &
                                                 none (\csp)& \cellcolor{Gray}- & \cellcolor{Gray}- & \cellcolor{Gray}- &\cellcolor{Gray}0\\
                &fast-forward    & \cellcolor{Gray}40  & \cellcolor{Gray}39  & \cellcolor{Gray}13 &\cellcolor{Gray}83\\
                &hard-coded      & \cellcolor{Gray}32 & \cellcolor{Gray}29 & \cellcolor{Gray}13 &\cellcolor{Gray}93\\
                &hand-crafted & 13 & 12 & 7 &100\\
        \midrule
        \multirow{4}{*}{Magic Square \(15\times15\)} &
                                                       none   (\csp)& 1.2 & 0.9 & 0.7 &100\\
                &fast-forward    & 1.7  & 1.2  & 1.5 &100\\
                &hard-coded      & 1.2 & 1.1 & 0.8 &100\\
        \midrule
        \multirow{4}{*}{Magic Square \(20\times20\)} &
                                                       none (\csp)& 5.1 & 4.6 & 1.8 &100\\
                &fast-forward    & 5.8 & 4.7 & 4.3 &100\\
                &hard-coded      & 3.8 & 2.8 & 2.7 &100\\
        \midrule
        \multirow{4}{*}{Magic Square \(25\times25\)} &
                                                       none (\csp)& 19 & 17 & 6.9 &100\\
                &fast-forward    & 12 & 10 & 9.7 &100\\
                &hard-coded      & 8.6 & 7.1 & 5.8 &100\\
        \midrule
        \multirow{4}{*}{Killer Sudoku \(9\times9\)} &
                                                      none (\csp)& \cellcolor{Gray}- & \cellcolor{Gray}- & \cellcolor{Gray}- &\cellcolor{Gray}0\\
                &fast-forward    & 3.9  & 3.3  & 2.6 &100\\
                &hard-coded      & 2.6 & 2.3 & 1.5 &100\\
                &hand-crafted & 1.6 & 1.6 & 0.8 &100\\
        % \midrule
...
network are  slightly more than  twice longer than run-times  of their
hard-coded version.

More  importantly,  we see  that  the  most frequently  learned  error
function by our system, once hard-coded in C++, finds solutions almost
as quickly as the carefully  hand-crafted error function from Petit et
al.~\cite{Petit2001}. Thus,  our method  can be used  to automatically
...
Interpretable  Compositional Networks,  a particluar  directed acyclic
...
Experiment~1 over small and complete training spaces, it finds the the
exact Hamming or Manhattan costs for 5 of them.  Their error functions
have  been  learned  over  constraint  assignment  space  composed  of
500\(\sim\)600  assignments  and  perfectly  scale  on  high-dimension
constraint instances with  \(10^{200}\) assignments.  In Experiment~2,
we show the robustness of our  system by learning error functions over
incomplete  constraint  assignment  space containing  20,000  randomly
drawn assignments  from spaces  of about \(10^{12}\)  assignments.  It
finds high-quality error functions for 4 out of 5 constraints that had
a perfect error function in the previous Experiment~1.
...
like  NoOverlap1D and  Ordered, experiments  show that  small training
spaces  are  too restricted  and  do  not  contain enough  of  diverse
assignments, and  the current  set of elementary  operations composing
our \icn model is certainly not expressive enough.
...
We made this system modular, allowing users with special needs can add
