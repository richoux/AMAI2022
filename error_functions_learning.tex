...
interpolation  and genetic  algorithms.  But  these approaches  failed
mainly because of  the same reason: expressing error  functions with a
sum of $p$ sinusoids led to either functions far from being satisfying
($p < 40$) or with too many coefficients to learn ($p \geq 40$).
...
(\cppn)~\cite{CPPN}, a variant of neural networks from which \(\icn\)s
are inspired  from.  Our  \cppn architecture  was a  two-layer network
composed of units with an  activation function among the identity, the
absolute value, the  sine function, the hyperbolic  tangent, the cubic
hyperbolic tangent, a sigmoid function, and a Gaussian function.
...
on
GitHub\footnote{\url{https://github.com/richoux/LearningErrorFunctions/tree/2.0} }.
...
space  of  size  \(100^{100}~=~10^{200}\), thus  greatly  larger  than
training spaces containing 500\(\sim\)600
assignments.  For some constraints, it was not possible to reach this
constraint assignment  space size  for a test  set.  We  explain which
constraints and why in Section~\ref{sec:results_xp1}. \flo{Ou alors maintenant.}
...
2 variables, the second a mean error one in 20 variables.
...
with  solution  rates from  \(0.15\)\%  to  \(2.10^{-7}\)\%. Then,  we
...
solution among  the 10,000  ones, and learn  error functions  on these
20,000 assignments  and their  estimated Hamming and  Manhattan costs.
Like for  Experiment~1, we run  100 error functions learning  of these
pre-sampled incomplete  spaces, so  that each  learning relies  on the
same training set.   Finally, we evaluate the  learned error functions
over the same test sets than Experiment~1.
...
representations of our constraints.  We have two baselines\flo{à revoir}: 1., a pure
\csp model  where constraints are  predicates, and 2., an  \efsp model
with an efficient hand-crafted error function for each constraint.  We
compare those with  two models using error functions  learned with our
system: a.,  our \efsp model  using the most frequently  learned error
function from  the previous  experiments and  computed by  running the
\icn graph in a fast-forward fashion, and b., our \efsp model with the
same error  function but directly  hard-coded in C++.  The  solver and
its parameters  remain the same:  the only  thing that is  modified in
these   four    different   models   is   the    expression   of   the
constraint.  Notice that for LinearSum, % LinearLessThan and
% LinearGreaterThan
...
resolutions of \(15\times15\), \(20\times20\) and \(25\times25\) Magic
Square grids.
...
\begin{figure}[h]
	...
	\begin{subfigure}[t]{0.3\linewidth}
		\centering
    \includegraphics[width=\linewidth]{./figs/sudoku}
		\caption{Sudoku \(9\times9\)}\label{fig:sudoku} 
	\end{subfigure}
  \hfill
	\begin{subfigure}[t]{0.3\linewidth}
		\centering
    \includegraphics[width=\linewidth]{./figs/magic_square}
		\caption{Magic Square \(3\times3\)}\label{fig:magic_square} 
	\end{subfigure}
  \hfill
	\begin{subfigure}[t]{0.3\linewidth}
		\centering
    \includegraphics[width=\linewidth]{./figs/killer_sudoku}
  ...
  \begin{center}
    \begin{minipage}{270pt}
      ...
      \begin{tabular}{@{}g|g|g|g|g@{}}
        ...
        NoOverlap-3-8-2 & 0.007 ~(34)& 0.007 & 0.009 & 0.003\\
        \rowcolor{white}
  ...
  \begin{center}
    \begin{minipage}{270pt}
      ...
      \begin{tabular}{@{}g|g|g|g|g@{}}
        ...
        NoOverlap-8-32-3 & 0.030 ~~(19)& 0.030 & 0.029 & 0.002\\
        \rowcolor{white}
    ...
    \begin{minipage}{\linewidth}
      ...
      \begin{tabular}{@{}c|g|g|g|g|g@{}}
        ...
        \rowcolor{white}
             ...
             &NoOverlap-10-35-3 & 0.062 & 0.062 & ~~0.060 & ~~~0.004\\
        \rowcolor{white}
        ...
        \rowcolor{white}
             ...
             &NoOverlap-10-35-3 & 0.050 & 0.041 & ~~0.041 & ~~~0.004\\
        \rowcolor{white}
...
practice, this problem  could be alleviate by letting  more than 200ms
...
error function), this is not a major issue for the actual usage of our
system.
...
our system  is overfitting  on thses training  sets, but  results from
Experiment~2 lead us to another conclusion.
...
Table~\ref{tab:test}  shows  us  that  our system  was  able  to  find
...
assignment spaces.   Obseve that the  learned error functions  for the
...
a poor choice for this constraint.
...
does  not contain  the  most trickier,  complex  assignments an  error
...
function, we implemented the \emph{primal graph based violation error}
of  AllDifferent from  Petit et  al.~\cite{Petit2001}.  This  function
simply outputs  the number of  couples with identical values  within a
given  assignment.  To  run  this experiment,  we  used the  framework
\ghost~\cite{GHOST},  which includes  a constraint-based  local search
algorithm able to handle both \csp and \efsp models.
    ...
    \begin{minipage}{\linewidth}
      ...
      \begin{tabular}{@{}l|c|r|r|r|r@{}}
        ...
        \multirow{4}{*}{Sudoku \(9\times9\)} &
                                               none (\csp)& \cellcolor{Gray}0.8 & \cellcolor{Gray}0.1 & \cellcolor{Gray}2.4 &\cellcolor{Gray}94\\
                &fast-forward    & \(2e^{-2}\)  & \(2e^{-2}\)  & \(8e^{-3}\) &100\\
                &hard-coded      & \(1e^{-2}\)  & \(1e^{-2}\)  & \(5e^{-3}\) &100\\
        ...
        \multirow{4}{*}{Sudoku \(16\times16\)} &
                ...
                &fast-forward    & 0.8  & 0.7  & 0.2 &100\\
                &hard-coded      & 0.5 & 0.4 & 0.1 &100\\
                &hand-crafted & 0.3 & 0.3 & 0.1 &100\\
        \midrule
        \multirow{4}{*}{Sudoku \(25\times25\)} &
                                                 none (\csp)& \cellcolor{Gray}- & \cellcolor{Gray}- & \cellcolor{Gray}- &\cellcolor{Gray}0\\
                &fast-forward    & \cellcolor{Gray}9.9  & \cellcolor{Gray}10  & \cellcolor{Gray}0.1 &\cellcolor{Gray}2\\
                &hard-coded      & \cellcolor{Gray}9.6 & \cellcolor{Gray}10 & \cellcolor{Gray}0.7 &\cellcolor{Gray}25\\
                &hand-crafted & \cellcolor{Gray}7.3 & \cellcolor{Gray}7.6 & \cellcolor{Gray}2.5 &\cellcolor{Gray}61\\
        \midrule
        \multirow{4}{*}{Magic Square \(15\times15\)} &
                                                       none   (\csp)& 0.6 & 0.4 & 0.5 &100\\
                &fast-forward    & 1.5  & 1  & 1.3 &100\\
                &hard-coded      & 0.7 & 0.5 & 0.5 &100\\
        \midrule
        \multirow{4}{*}{Magic Square \(20\times20\)} &
                                                       none (\csp)& 2.8 & 2.4 & 1.6 &100\\
                &fast-forward    & \cellcolor{Gray}3.3  & \cellcolor{Gray}2.1  & \cellcolor{Gray}2.8 &\cellcolor{Gray}95\\
                &hard-coded      & \cellcolor{Gray}2.8 & \cellcolor{Gray}2 & \cellcolor{Gray}2.5 &\cellcolor{Gray}96\\
        \midrule
        \multirow{4}{*}{Magic Square \(25\times25\)} &
                                                       none (\csp)& \cellcolor{Gray}9.8 & \cellcolor{Gray}10 & \cellcolor{Gray}0.6 &\cellcolor{Gray}12\\
                &fast-forward    & \cellcolor{Gray}8.3  & \cellcolor{Gray}10  & \cellcolor{Gray}2.4 &\cellcolor{Gray}37\\
                &hard-coded      & \cellcolor{Gray}7.9 & \cellcolor{Gray}10 & \cellcolor{Gray}2.6 &\cellcolor{Gray}49\\
        \midrule
        \multirow{4}{*}{Killer Sudoku \(9\times9\)} &
                                                      none (\csp)& \cellcolor{Gray}- & \cellcolor{Gray}- & \cellcolor{Gray}- &\cellcolor{Gray}0\\
                &fast-forward    & 2  & 1.6  & 1.5 &100\\
                &hard-coded      & 1.2 & 0.9 & 1.1 &100\\
                &hand-crafted & 1.1 & 0.8 & 1 &100\\
        % \midrule
...
Table~\ref{tab:sudoku} shows that models  with error functions clearly
outperformed  the   model  with   the  constraint  represented   as  a
predicate. Over 100  runs, no error function-based models  hit the 60s
timeout, but 4 runs of the  regular constraint network model timed out
on  the \(9\times9\)  grid,  and  all of  them  on the  \(16\times16\)
grid. Moreover, the learned error function hard-coded in C++ is nearly
as  efficient  as the  hand-crafted  one  (also  coded in  C++).   The
difference of  runtimes between the learned  error function hard-coded
and computed  through the  \icn gives  us an idea  of the  overhead of
computing such a function through the \icn.

%%%%%%%%%
Table~\ref{tab:sudoku} shows that error functions are well exploited
by the  solver.  We  run 100  solving of Sudoku  for each  model and
compute the mean and median run-time in milliseconds, as well as the
standard deviation, the shortest and the longest run-time: run-times
from error  function-based models are significantly  better than the
pure  \csp  model  ones.   We  can also  estimate  the  overload  of
computing the error function through the interpretable compositional
network (ran  in a  feed-forward fashion),  compare to  a hard-coded
version of the same error function.  We recall that one advantage of
our method  is to output  intelligible error functions,  letting the
choice to users  to compute this function  through the Interpretable
Compositional  Network or  to let  them the  possibility to  code it
themselves.   Results  from  Table~\ref{tab:sudoku}  show  that  the
overload is such that run-times  of error functions executed through
the interpretable compositional network are slightly more than twice
longer than run-times of their hard-coded version.

More  importantly,  we  see  that the  most  frequently  learned  error
function trying to  reproduce the Hamming cost  finds solutions almost
as   quickly    as   the   carefully   hand-crafted    error   function
from Petit et al.~\cite{Petit2001}. Thus, our method can be used to automatically
find error  functions with an equivalent  efficiency than human-defined
ones.
...
\begin{equation*}
  \begin{array}{ll}
    ...
    NoOverlap: &\(Max\Big( 0, \sum_{i=1}^n\left(Count_{=}^{l}(\vec x) + Count_{<+p}(\vec x)\right) - n \Big)\)\\
...
\begin{equation*}
  \begin{array}{ll}
    ...
    NoOverlap: &\(Max\Big( 0, \sum_{i=1}^n\left(Count_{>=<+p}(\vec x) + Count_{<+p}(\vec x)\right) - p \Big)\)\\
